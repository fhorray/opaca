...
function buildBaseTemplateFiles(opts: ScaffoldOptions): Record<string, string> {
  ...
  const pkg = {
    ...
    devDependencies: {
      "@opaca/dev": DEFAULTS.bunaDev,
      "@opaca/devtools": "latest",
...
`;
  ...
  const bunaConfig = `import { defineConfig } from "opaca";
  ...
  return (
    <main className="min-h-screen bg-[#0d0d0d] text-slate-100 flex items-center justify-center px-6">
      <div className="w-full flex justify-center flex-col gap-4 max-w-xl text-center">
        <img src={logoUrl} alt="Opaca logo" width={100} className="self-center" />
        ...
        <p className="mt-10 text-xs text-slate-500">Built using opaca() + Tailwind</p>
      </div>
    </main>
  );
});
...
Home.meta = {
  title: "Welcome to opaca()",
  description: "Starter page for your Opaca app.",
...
A minimal ${opts.runtime === "cloudflare" ? "Cloudflare Worker" : "Bun"} app bootstrapped with opaca().
...
- \`bun run build\` ${isCloudflare
      ? "(builds the Cloudflare worker at .opaca/cloudflare)"
      : "(builds your Bun server)"
    }.
- \`bun run opaca:prepare\` regenerates .opaca helpers (routes + build runner).
