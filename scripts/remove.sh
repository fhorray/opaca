...
if [ -z "$PKG_NAME" ] || [ -z "$LIBS" ]; then
  ...
  echo -e "   ${GRAY}usage:${RESET} bun remove ${CYAN}<workspace>${RESET} ${YELLOW}<lib...>${RESET}"
  echo -e "   ${GRAY}example:${RESET} bun remove core zod nanostores"
  exit 1
fi
...
bun remove $LIBS --cwd "$TARGET_PATH"
EXIT_CODE=$?

echo -e ""
if [ $EXIT_CODE -eq 0 ]; then
  echo -e "${GREEN}✔${RESET} ${BOLD}Done${RESET} ${GRAY}(bun remove completed successfully)${RESET}"
else
  echo -e "${RED}✖${RESET} ${BOLD}bun remove failed${RESET} ${GRAY}(exit code: $EXIT_CODE)${RESET}"
fi
